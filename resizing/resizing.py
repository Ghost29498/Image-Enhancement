...
class Resizing:
	def __init__(self,modelPath,device):
        ...
        Parameters:
			...
			device: 'Device to work on, cpu/gpu'
        Architecture
		...
		nf = 64
		nb = 23
		model = arch.RRDBNet(in_nc, out_nc, nf, nb)
		model.load_state_dict(torch.load(modelPath), strict=True)
		model.eval()
		self.diffThreshold = 600
		self.device = device
		self.model = model
	...
	def blurOut(self,image,portion):
		...
		kernelMask = 55
		iterations = 7
		img1 = softBlurRect(image,rect1, kernelSize,sigmaX,sigmaY,kernelMask, iters = iterations)
		img2 = softBlurRect(img1,rect2, kernelSize,sigmaX,sigmaY,kernelMask, iters = iterations) 
		img3 = softBlurRect(img2,rect3, kernelSize,sigmaX,sigmaY,kernelMask, iters = iterations) 
		img4 = softBlurRect(img3,rect4, kernelSize,sigmaX,sigmaY,kernelMask, iters = iterations)
		return(img4)
	...
	def shrink(self,img,newHeight,newWidth,height,width):
		...
		background = resize(im,newHeight,newWidth)
